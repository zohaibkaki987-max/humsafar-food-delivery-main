...
$slots = [
    ...
    ['21:00:00', '00:00:00', '9:00 PM - 12:00 AM'],
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['toggle_session'])) {
    ...
    if (!$isApproved) {
        ...
        /*
         * 12 AM - 3 AM belongs to the next calendar day.
         */
        $sessionStart = new DateTimeImmutable(
            $date . ' ' . $start
        );

        if ($start === '00:00:00') {
            $sessionStart = $sessionStart->modify('+1 day');
        }

        /*
         * Do not allow booking sessions that have already started.
         */
        $now = new DateTimeImmutable('now');
        ...
        if (
            ...
            $error = 'This session has already started or passed. Please select a future session.';
...
foreach ($slots as $slotIndex => $slot):
    ...
    $now = new DateTimeImmutable('now');
    $bookingDeadline = $sessionStart->modify('-30 minutes');
    $isPast = $now >= $bookingDeadline;
?>
<div class="slot <?=$isSelected?'selected':''?> <?=(!$isApproved || $isPast)?'locked':''?>">
...
<div class="slot-status">
    ...
    if ($isSelected) {
        ...
        echo 'Available session';
    }
    ?>
</div>
<form method="post"><input type="hidden" name="toggle_session" value="1"><input type="hidden" name="date" value="<?=rs_e($date)?>"><input type="hidden" name="slot" value="<?=$slotIndex?>"><input type="hidden" name="action" value="<?=$isSelected?'remove':'add'?>">
<button
    class="<?=$isSelected?'remove':'book'?>"
    type="submit"
    <?=(!$isApproved || ($isPast && !$isSelected)) ? 'disabled' : ''?>
>
    <?=$isSelected ? 'Remove Session' : ($isPast ? 'Session Started' : 'Book Session')?>
</button>
</form>
</div>
<?php endforeach; ?>
