...
<section class="card"><h3><i class="fa-solid fa-store"></i> Restaurant Commission</h3><p class="hint">Separate from customer markup. Used for restaurant settlement.</p><form method="post"><input type="hidden" name="action" value="setting"><input type="hidden" name="key" value="restaurant_commission_percent"><div class="field"><label>Restaurant Commission (%)</label><input class="input" name="value" type="number" min="0" max="100" step="0.01" value="<?=bh($commission)?>" required></div><button class="btn">Save Commission</button></form><div class="formula current-row"><span>Current Commission</span><span class="badge"><?=$commission?>%</span></div></section>
<section class="card setting-card"><h3><i class="fa-solid fa-motorcycle"></i> Delivery Fee Control</h3><p class="hint">Admin sets one delivery rate per started KM. This is the <b>global rate for every restaurant</b>.</p><form method="post"><input type="hidden" name="action" value="setting"><input type="hidden" name="key" value="delivery_fee_per_km"><div class="field"><label>Delivery Fee per KM (PKR)</label><input class="input" name="value" type="number" min="0" step="0.01" value="<?=bh($rate)?>" required></div><button class="btn" type="submit"><i class="fa-solid fa-floppy-disk"></i> Save Delivery Fee</button></form><div class="formula"><b>Current:</b> <?=$rate?> PKR per started KM<br><small>Example: 3.2 KM = 4 KM × <?=$rate?> PKR.</small></div></section>
<section class="card setting-card"><h3><i class="fa-solid fa-wallet"></i> Rider Payout Control</h3><p class="hint">Admin sets the fixed amount a rider earns for <b>each completed delivery</b>.</p><form method="post"><input type="hidden" name="action" value="setting"><input type="hidden" name="key" value="rider_base_payout"><div class="field"><label>Rider Payout per Delivery (PKR)</label><input class="input" name="value" type="number" min="0" step="0.01" value="<?=bh($riderPay)?>" required></div><button class="btn" type="submit"><i class="fa-solid fa-floppy-disk"></i> Save Rider Payout</button></form><div class="formula"><b>Current Rider Payout:</b> <?=$riderPay?> PKR per completed order.</div></section>
<section class="card" style="grid-column:1/-1"><h3>Pricing Model</h3><div class="formula"><b>Customer item price</b> = Restaurant base price × (1 + Customer Markup % / 100)<br><b>Delivery fee</b> = CEIL(distance in KM) × Admin Global Delivery Fee/KM<br><b>Customer total</b> = Marked-up items + Delivery fee − Coupon discount<br><b>Rider payout</b> = Admin Rider Payout per completed delivery<br><b>Restaurant settlement</b> = applicable restaurant base amount − Restaurant Commission</div></section>
